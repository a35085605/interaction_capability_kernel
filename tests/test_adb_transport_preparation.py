...
def _row(state: AdbConnectionState | int, transport_id: int = 27) -> AdbTrackedDevice:
    return AdbTrackedDevice(serial="device-1", state=state, transport_id=transport_id)
...
class _Connector:
    def __init__(self, result: NativeAttemptResult, after_connect=None) -> None:
        self.result = result
...
class AdbTransportPreparationOrchestratorTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.session_id = AdbObservationSessionId(_endpoint(), 7)
        self.observation = _Observation(self.session_id)
        self.operation = AdbTransportPreparation(_endpoint(), AdbDeviceSerial("device-1"))
        self.policy = AdbTransportPreparationPolicy(
            timeout_seconds=0.2,
            acceptable_states=frozenset({AdbConnectionState.DEVICE}),
            blocked_states=frozenset({AdbConnectionState.UNAUTHORIZED, AdbConnectionState.NOPERMISSION}),
        )
    ...
    def test_already_ready_transport_skips_connect(self) -> None:
        ...
        result = self._orchestrator(
            AdbDevicesSnapshot((_row(AdbConnectionState.DEVICE),)), connector
        ).prepare(self.operation, self.policy)
        self.assertIs(result.status, AdbTransportPreparationStatus.SATISFIED)
        self.assertIs(result.satisfaction, AdbTransportPreparationSatisfaction.ALREADY_SATISFIED)
        self.assertIs(result.presence_satisfaction, AdbTransportPresenceSatisfaction.ALREADY_PRESENT)
    ...
    def test_presence_and_state_are_two_gates_inside_one_episode(self) -> None:
        ...
        result = self._orchestrator(AdbDevicesSnapshot(), connector).prepare(self.operation, self.policy)
        self.assertIs(result.status, AdbTransportPreparationStatus.SATISFIED)
        self.assertIs(result.presence_satisfaction, AdbTransportPresenceSatisfaction.OBSERVED)
    ...
    def test_initial_presence_can_achieve_readiness_later_without_connect(self) -> None:
        connector = _Connector(_attempt())
        orchestrator = self._orchestrator(AdbDevicesSnapshot((_row(AdbConnectionState.OFFLINE),)), connector)
        ...
        self.assertIs(result.status, AdbTransportPreparationStatus.SATISFIED)
        self.assertIs(result.satisfaction, AdbTransportPreparationSatisfaction.ACHIEVED)
        self.assertIs(result.presence_satisfaction, AdbTransportPresenceSatisfaction.ALREADY_PRESENT)
        self.assertEqual(connector.operations, [])
    ...
    def test_blocked_state_terminates_by_explicit_policy(self) -> None:
        connector = _Connector(_attempt())
        result = self._orchestrator(
            AdbDevicesSnapshot((_row(AdbConnectionState.UNAUTHORIZED),)), connector
        ).prepare(self.operation, self.policy)
    ...
    def test_serial_selected_preparation_follows_fresh_resolution(self) -> None:
        ...
        orchestrator = self._orchestrator(
            AdbDevicesSnapshot((_row(AdbConnectionState.OFFLINE, 27),)), connector
        )
        original_read = orchestrator._snapshot_reader.read
    ...
    def test_newer_observation_generation_terminates_episode(self) -> None:
        ...
        result = orchestrator.prepare(self.operation, self.policy)
        self.assertIs(result.status, AdbTransportPreparationStatus.OBSERVATION_REPLACED)
